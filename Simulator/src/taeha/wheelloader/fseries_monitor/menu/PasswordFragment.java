...
public abstract class PasswordFragment extends ParentFragment{
	...
	public void CheckPassword_Nocan(){
		Log.d(TAG,"CheckPassword_Nocan Result"+DataBufIndex);
		
		// ++, 150323 bwk
		int TempNumData_User[]={0x30,0x30,0x30,0x30,0x30};
		int TempNumData_Service[]={0x36,0x35,0x38,0x37,0x31,0x31,0x32,0x36};
		int TempResult = 1;
		for(int i=0;i<DataBufIndex;i++)
		{
			if(DataBufIndex == 5)
			{
				if(NumDataBuf[i] != TempNumData_User[i])
				{
					TempResult = 0;
					break;
				}
			}
			else if(DataBufIndex == 8)
			{
				if(NumDataBuf[i] != TempNumData_Service[i])
				{
					TempResult = 0;
					break;
				}
