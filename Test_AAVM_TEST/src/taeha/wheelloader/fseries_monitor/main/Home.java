...
public class Home extends Activity {
	...
	// // 1.0.2.5
	// Error Report 추가 (경로 : mnt/sdcard/alarams/WheelLoader_Logxxx.txt)
	// 2014.12.12
	...
	// 소모품 관리 Hyd. Tank Air Breatehr Filter Cartridge 에서 Cartridge 단어 삭제
	// 2014.12.30
	...
	// Engine Auto Shutdown Countdown 중 Cancel 버튼 클릭 시 61184_121(SPN363) Off로
	// 설정해서 Tx하게 수정 2014.12.31
	// Eco Gauge 색상 변경, Yellow, Red 삭제 2015.01.05
	// Cooling Fan Manual 버튼 누른 후 Off로 돌아가기 -> Manual 진입 후 나갈 때, Auto로 돌아가는 것으로
	// 수정 2015.01.05
	...
	// Boom pressure Calibration : HCESPN1908 = 14 추가 및 UI 변경(작동유 온도 표시 및 경고문구
	// 추가)
	// F시리즈 통신개통 자동화(장비고장 페이지에서 Screen Number 전송, 그외 0xff) + 키값전송(ST 1.0.1.9),
	// 현재고장 페이지에서 Home키 막음
	// 메인 화면의 작업량 계측값 표시부 : 통신사양변경, Icon추가, 가이던스 추가, Enter로 reweigh하도록 로직 변경
	// 키패드 히든 기능 관련 : < + > + Enter -> 다국어
	// EHCU POP UP 관련 반영 : Safety CPU Error 팝업일 경우 Keyoff전에 꺼지지 않음. 0000일 경우 팝업
	// 종료
	// 다국어 적용 : 한국어만 적용, 다국어 페이지 추가
	// speedometer freq. setting : <- 키 오류 수정
	// Work Load : 키패드 버튼 UI일 때 Default 팝업창 추가, 붐압력 보정항목 선택후 이전 화면으로 이동(Main은
	// 해당페이지가 아닌 main으로 이동)
	...
	// 키패드 버튼 UI : rear wiper : keypad longkey 입력 시 washer 동작 intermittant, 이 후
	// 이전값으로 이동되도록 수정
	// Intro/Outro 적용 (Intro : OS 1.0.6)
	// 냉각팬 설정 : Manual의 Execute 버튼에 대한 설명 문구 적용 (‘push and hold to reverse
	// rotation’)
	...
	// 5. 다국어 적용(총 21개 언어)
	// - 아랍어 List 부분 오른쪽 정렬되는 부분 모두 왼쪽 정렬로 변경(List, 단위변환, Workload, Soft End
	// Stop)
	...
	// - 버전정보에서 'EHCU', 'TCU', 'Machine', 'Transmission', 'Engine' 다국어 적용 시 문구가
	// 짤리는 경우가 발생하여
	...
	// 6. 스마트키 사용안함 + 시동제한 일 경우 부팅 시 시동제한 비밀번호 안뜨는 버그 수정(사용안함일 경우 ST로 보내는 부분이
	// 주석되어있었음)
	// //1.0.3.6(15.03.07)
	...
	// 4. Main : Left Down => Average Fuel, Lastest Fuel Consumed 추가,
	// Hourmenter, Odometer 제거
	// 5. 도움말 글씨 하늘색으로 변경
	...
	// - MCU - 소모품 관리 기능 : 소모품 항목별 경고 램프 점등 / 소모품관리 경고 램프 점등 각각의 갱신 타이밍을 동일하게
	// 가져가고자 함
	// - maintenance 알람
	...
	// - 메뉴 내 장비 상태(Machine monitoring) 표시 기능(온도, 배터리 레벨 등)에 액슬 온도 항목 추가 (옵션 미장착
	// 시 감춤)
	// - 프로토콜 신규 추가
	// - 액승 오일 경고 프로토콜 추가
	...
	// 9. Rear Wiper 버튼 누를 때마다 상태가 변경되어야 하는지(현재 어느 상태이든 누르면 Intermittent로 변경됨)
	// -> 버그 수정
	// 10. Work Load : 내부 Boom PS calibraion 항목 선택 후, ESC 누르면 Home으로 이동됨 ( 이전
	// 항목으로 이동되도록 변경)
	...
	// 1) EHCU 인식하여 EHCU 미적용시 다음 메시지 출력 : “EH SYSTEM is NOT equiped.” : 영어로만 표시
	// 합니다.
	...
	// if(ParentActivity._CheckModel.GetMCUVersion(CAN1Comm.Get_ComponentBasicInformation_1698_PGN65330())
	// == CheckModel.MODEL_940
	// ||
	// ParentActivity._CheckModel.GetMCUVersion(CAN1Comm.Get_ComponentBasicInformation_1698_PGN65330())
	// == CheckModel.MODEL_935)
	// - 변경 후 : EHCU CID 판별
	// if(CAN1Comm.Get_ComponentCode_1699_PGN65330_EHCU() !=
	// CAN1CommManager.STATE_COMPONENTCODE_EHCU)
	...
	// - Auto Grease, Wuick coupler, Ride control, Beacon lamp, Mirror heat,
	// Fine Modulation
	...
	// 3. 과거 고장 삭제 팝업에서 ESC 눌렀을 경우 과거고장 페이지에서 커서 잃는 버그 수정(Crash -> UpdateUI사용으로
	// 변경)
	...
	// 18. 버전 정보 표시화면 변경
	// - 버전정보 -> 장비정보 이름 변경(한국어)
	// - ECM 표시내용 : 제조사, ECM Identifier(성현균
	// 대리)->Calibration_Version_Number(임혁준씨)
	...
	// 3. 가상키 Not available일 경우 회색 음영 처리(Auto grease, Quick coupler, Ride
	// control, Beacon lamp, Mirror heat)
	...
	// 5. 페이지 이동 후 라이오버튼 클릭으로 나오는 페이지일 경우 초기 애니메이션 중 빠르게 다른 항목 누를 경우 이중 선택 혹은
	// 페이지에서 나오지 못하는 현상 발생
	// -> 애니메이션이 완료되면 화면을 활성화
	// - 다국어 설정
	// - 메인(Fuel Select, ODO/HOURMETER Select, Engine Mode, CCO/ICCO Mode, Shift
	// Mode, T.C.Lock Up)
	// 6. 다국어 - 한국어, 영어 최종본 적용
	...
	// 14. Quick Coupler LED 꺼지지 않는 현상 확인 후 프로토콜 추가(2에 대한 사양이 없었음 -> MCU LED OFF
	// 사양으로 확인 됨)
	...
	// HCEPGN 65524 (4~5) : 0x0000 - Joystick Steering Enable OK -> 0x0000 - Not
	// Available
	...
	// 4. Main A Quick에서 소모품관리/UserSwitch/현재고장 페이지 이동하였다가 Back하였을 경우 가상키패드 안되는
	// 현상 해결
	// 5. UserSwitching
	...
	// 3. 붐/버켓 각도보정, 붐 압력보정 기능 - EHCU CID 인식하여 EHCU 장착 장비에서는 최초 화면에 추가 문구 or 팝업
	// 표시
	...
	// 2. Software Update 개선방안 준비중(시동 중일 경우 Software Update를 하지 않고 경고 팝업만 띄움(Key
	// on시에만 업데이트 되도록))
	...
	// - Main : FuelSelect, MachineStatusSelect,
	// TMCCOMode/ICCOMode/ShiftMode/TCLockUp, EngineMode, HourOdometerSelect
	// 7. 10초 복귀 중일 때 미적용된 페이지 버튼 누를 경우에도 복귀되는 버그 수정
	// 8. 역회전 관련 버그 개선
	// - [실행] 커서 선택 후 ENTER 키 입력 -> 이때 역회전 터치시 방향키 작동 안됨 -> CurserIndex 추가
	// - [실행] 커서 선택 후 ENTER 키 입력 -> 이때 ESC 키 입력 시 수동 역회전 꺼지지 않고 계속 동작 -> 팝업꺼질 경우
	// OFF 프로토콜 송부
	// // v2.0.0.8
	// 1. 냉각팬 UI 변경 및 로직 변경
	...
	// - 마지막에 3채널로 해놓고 카메라를 OFF 했을 경우 초기 1 -> Right 키를 누르면 CH4로 변경되는 버그 수정(항상
	// 첫번째 카메라를 보여줌)
	...
	// 10. pdf reader, media player, 스마트 터미널 등 기능에서 Axle 경고(pop up 또는 lamp)가 발생할
	// 시 바로 종료 되고 메인화면으로 전환
	// // v2.0.0.91
	...
	// 3. HCEPGN 65373 Air Conditioner Status : 관리자 메뉴 - 장비 모니터링에 표시 항목(FATC
	// Setting Temperature (Celsius)) 추가
	// 4. HCEPGN 65519 Ambient Conditions : 관리자 메뉴 - 장비 모니터링에 표시 항목(Ambient
	// Temperature, In-cab Temperature) 추가
	...
	// - 우선 순위 중간
	// a. Engine SCR Temp High, Axle Temp, Joystick EHCU > Quickcoupler Unlock
	// Alarm
	...
	// - 자동정지 팝업을 우선 처리하고, EHCU error 팝업 관련 플래그 리셋.(“Please reset power” 팝업이 다시
	// 나타나도록)
	// B. 그 외 Joystick Steering 관련 팝업 시에도 마찬가지로 자동정지 팝업을 우선 처리.
	// 2. 편의용 App(PDF Viewer, Media Player, Smart Terminal) 구동 중 경고 표시
	// A. 다음 경고 표시 발생 시 모니터 메인화면으로 복귀할 것 (구동중이던 App은 백그라운드 동작, 단 PDF Viewer는 종료
	// 됨)
	...
	// //v2.1.0.20
	// 1. T.C Lock Up 사양 변경(모델정보는 비교하지 않고, TCU 정보로만 판별(0x20, 숫자만 유효데이터로 함).
	// Default값은 5Speed)
	...
	// 7. Fuel 팝업 초기화 중 팝업이외 클릭 시 crash 뜨는 현상 및 초기화와 OK 버튼 사이 클릭 시 UI 색상 바뀌는 현상
	// 개선
	...
	// v.2.5.1.0
	// 1. AAVM 기능 추가
	// 2. Multimedia 제한 기능 추가
	// 3. UserSwitching 잠금 기능 추가
	// 4. Smart key 기능 개선
	// v.2.5.1.1
	// 1. 혁준씨 요청으로 인한 AAVM - Check flag 반대로
	// v.2.5.1.2 AAVM_TEST
	//	1.	AAVM 추가 기능 건 - 카메라 화면, DTC(현재고장, 과거고장), 버전 정보(CID 표시) 기능.
	//		A.	후진기어 연동 카메라에서는 AAVM 화면 선택 버튼 미동작 : 동작하도록 수정
	//		B.	버튼 사라졌을 시 화면 터치로 버튼 나타나는 기능 : 터치로 버튼 사라짐도 되도록 할 것
	//		C.	버튼 관련 조작이 있을 때마다 화면 전체가 깜빡임 : 깜빡이지 않도록 수정 -> 수정
	//		D.	우측 모서리의 2D 화면 선택 버튼 4개 간의 간격을 줄일 것 (4분할 영상 선택 버튼과의 간격이 너무 짧음) : 칸에 맞춰져 있어 이미지를 줄여야함.
	//	         -> 이미지 송부
	//		E.	후방카메라 진입 시 POP UP 메시지 : 다시 보지 않기 기능 체크박스 형식으로 추가
	//		F.	모니터 키패드로 화면 선택 버튼 간 커서 이동 기능 : 최초 카메라 화면 진입시 터치 안하면 커서가 안보임 : 해결
	//	2.	멀티미디어 제한 기능
	//		A.	멀티미디어/스마트 터미널 작동 중 화면 전환 후 제한기능 적용 할 시에 계속 켜져있음 : OFF 하도록 수정 -> OFF는 되는데 LED 안꺼짐...
	//		B.	멀티미디어/스마트 터미널 모두 제한 후 특정 상황에서 멀티미디어 화면에 진입 가능 (유첨 사진 참조)
	//			i.	진입 조건 : 메뉴, 기능 설정 화면에서 탭 레벨에서 커서를 옮기고 esc로 메뉴 레벨에서 1회 위로 움직여서 멀티미디어 화면에 진입 _
	//	3.	User Switching 잠금 항목 설정 기능 
	//		A.	붐/버켓 디텐트 모드 잠금 시 save position도 잠기도록 수정z
	//		B.	화면 표시 이상 : 아래 기타 B 참조
	//	5.	기타
	//		A.	CID 화면 개선 : 그대로 적용.
	//		B.	일부 UI에서 text 혼선 발생, string 파일 등 재확인 필요 : clock setting이 두개, language가 type A로 표시, camera setting이 (reverse camera)로 표시 (유첨 사진 참조)
	//		C.	현재고장, 과거고장 표시 기능에서 에러코드 수가 FF(255)로 들어올 경우, 오류로 판단하여 (255)대신 (-) 로 표시할 것
	// v.2.5.1.2
	// 1. CAM 경고 팝업 다시보지 않기기능 (Key Off 하면 초기화)
	// v.2.5.1.3
	// 1. 후진기어 연동 유지
	// 2. 후진기어 연동 화면 타입 선택하는 기능 추가
	// 3. AAVM 카메라 표시화면 CH4로 고정 / 보행자 감지 경고 추가
	// 4. 화면타입 선택 버튼 터치 영역 증가
	// 5. 커서 이동 직후 엔터 입력에도 CAN 송신
	// ////////////////////////////////////////////////////////////////////////////////////
	// TAG
	...
	// ++, 150316 bwk
	// public static final int SCREEN_STATE_MAIN_B_RIGHTUP_ENGINE_WARMINGUP =
	// 0x11200000;
	...
	// ++, 150330 bwk
	// public static final int SCREEN_STATE_MAIN_B_LEFTUP_MACHINESTATUS =
	// 0x13100000;
	...
	// ++, 150317 bwk
	// public static final int SCREEN_STATE_MAIN_B_LEFTDOWN_HOURODOMETER =
	// 0x14100000;
	...
	// public static final int SCREEN_STATE_MENU_MODE_ETC_COOLINGFAN_OFF =
	// 0x21321000;
	// public static final int SCREEN_STATE_MENU_MODE_ETC_COOLINGFAN_AUTO =
	// 0x21321000;
	// public static final int SCREEN_STATE_MENU_MODE_ETC_COOLINGFAN_MANUAL =
	// 0x21322000;
	...
	// public static final int SCREEN_STATE_MENU_MODE_ETC_CAMERASETTING_TOP =
	// 0x21350000;
	// public static final int SCREEN_STATE_MENU_MODE_ETC_CAMERASETTING_END =
	// 0x2135FFFF;
	...
	// ++, 150329 bwk
	// public static final int SCREEN_STATE_MENU_MONITORING_MACHINEMONITORING =
	// 0x22100000;
	...
	// ++, 150329 bwk
	// public static final int SCREEN_STATE_MENU_MONITORING_EHCUINFO_TOP =
	// 0x22500000;
	// public static final int
	// SCREEN_STATE_MENU_MONITORING_EHCUINFO_BOOMLEVERFLOAT = 0x22510000;
	// public static final int SCREEN_STATE_MENU_MONITORING_EHCUINFO_END =
	// 0x225FFFFF;
	// --, 150329 bwk
	...
	// public static final int SCREEN_STATE_MENU_MANAGEMENT_CALIBRATION_TOP =
	// 0x23300000;
	// public static final int
	// SCREEN_STATE_MENU_MANAGEMENT_CALIBRATION_ANGLE_TOP = 0x23310000;
	// public static final int
	// SCREEN_STATE_MENU_MANAGEMENT_CALIBRATION_ANGLE_RESULT = 0x23311000;
	// public static final int
	// SCREEN_STATE_MENU_MANAGEMENT_CALIBRATION_ANGLE_END = 0x2331FFFF;
	// public static final int
	// SCREEN_STATE_MENU_MANAGEMENT_CALIBRATION_PRESSURE_TOP = 0x23320000;
	// public static final int
	// SCREEN_STATE_MENU_MANAGEMENT_CALIBRATION_PRESSURE_RESULT = 0x23321000;
	// public static final int
	// SCREEN_STATE_MENU_MANAGEMENT_CALIBRATION_PRESSURE_END = 0x2332FFFF;
	// public static final int
	// SCREEN_STATE_MENU_MANAGEMENT_CALIBRATION_BRAKEPEDAL_TOP = 0x23330000;
	// public static final int
	// SCREEN_STATE_MENU_MANAGEMENT_CALIBRATION_BRAKEPEDAL_END = 0x2333FFFF;
	// public static final int SCREEN_STATE_MENU_MANAGEMENT_CALIBRATION_AEB_TOP
	// = 0x23340000;
	// public static final int SCREEN_STATE_MENU_MANAGEMENT_CALIBRATION_AEB_END
	// = 0x2334FFFF;
	// public static final int SCREEN_STATE_MENU_MANAGEMENT_CALIBRATION_END =
	// 0x233FFFFF;
	...
	// public static final int
	// SCREEN_STATE_MENU_MANAGEMENT_SERVICE_SOFTWAREUPATE_TOP = 0x23550000;
	// public static final int
	// SCREEN_STATE_MENU_MANAGEMENT_SERVICE_SOFTWAREUPATE_END = 0x2355FFFF;
	...
	public static final int SCREEN_STATE_MENU_PREFERENCE_SOUNDOUTPUT_CHANGE = 0x24510000; // ++,
	// --,
	// 150323
	// bwk
	...
	// ++, 151005 cjg
	/*
	 * public static final int STATE_DISPLAY_LANGUAGE_CHINESE = 13; public
	 * static final int STATE_DISPLAY_LANGUAGE_RUSIAN = 14; public static final
	 * int STATE_DISPLAY_LANGUAGE_THAILAND = 15; public static final int
	 * STATE_DISPLAY_LANGUAGE_HINDI = 16; public static final int
	 * STATE_DISPLAY_LANGUAGE_MONGOL = 17; public static final int
	 * STATE_DISPLAY_LANGUAGE_ARABIC = 18; public static final int
	 * STATE_DISPLAY_LANGUAGE_FARSI = 19; public static final int
	 * STATE_DISPLAY_LANGUAGE_INDONESIAN = 20;
	 */
	// --, 151005 cjg
	// --, 150206 bwk
	...
	public static int STATE_BRIGHTNESS_MANUALAUTO = 14; // ++, --, 150407 bwk 수동
	// 밝기 -> 수동/자동으로 변경
	...
	public static int STATE_OPERATION_HISTORY = 26; // ++, --, 150403 bwk 명칭 변경
	// STATE_HOURMETER ->
	// STATE_OPERATION_HISTORY
	public static int STATE_FUEL_INFO = 27; // ++, --, 150403 bwk 항목 추가
	...
	public void LoadPref() {
		...
		FuelIndex = SharePref.getInt("FuelIndex", CAN1CommManager.DATA_STATE_AVERAGE_FUEL_RATE); // ++,
																									// --,
																									// 150331
		// bwk // ++,
		// --, 150407
		// bwk 초기값 평균연비
		MachineStatusUpperIndex = SharePref.getInt("MachineStatusUpperIndex", CAN1CommManager.DATA_STATE_MACHINESTATUS_COOLANT); // ++,
																																	// --,
		// 150407
		// bwk 9A
		// 동일하게(NoSelect
		// -> 작동유)
		MachineStatusLowerIndex = SharePref.getInt("MachineStatusLowerIndex", CAN1CommManager.DATA_STATE_MACHINESTATUS_BATTERY); // ++,
																																	// --,
		// 150407
		// bwk 9A
		// 동일하게(NoSelect
		...
		InternalSoundLevel = SharePref.getInt("InternalSoundLevel", INTERNAL_SPK_MAX); // ++,
																						// --,
																						// 150324
																						// bwk
		...
		strASNumDash = SharePref.getString("strASNumDash", "1899-7282"); // ++,
		// --,
		// 150402
		// bwk
		// A/S
		// 번호
		// 추가
		strASNum = SharePref.getString("strASNum", "18997282"); // ++, --,
		// 150402 bwk
		// A/S 번호 추가
		...
		LanguageIndex = SharePref.getInt("LanguageIndex", STATE_DISPLAY_LANGUAGE_ENGLISH); // ++,
																							// --,
																							// 150206
																							// bwk
	...
	public UserData LoadUserData(int Index) {
		...
		// _userdata.WarmingUp = SharePref.getInt(strWarmingUp,
		// CAN1CommManager.DATA_STATE_ENGINE_WARMINGUP_OFF);
	...
	ICAN1CommManagerCallback mCallback = new ICAN1CommManagerCallback.Stub() {
		...
		public void KeyButtonCallBack(int Data) throws RemoteException {
			...
			// --, 150323 cjg
				...
				// // ++, --, 150326 cjg multimedia ending 개선
				// ++, 150615 cjg
				try {
		...
		public void CIDCallBack() throws RemoteException {
			...
			CAN1Comm.TxCMDToMCU(CAN1CommManager.CMD_RTC, ComponentBasicInformation[0], ComponentBasicInformation[1], ComponentBasicInformation[2], 0x01, Hour,
					Min, Sec, 0x00);
		}
	...
	public void CameraDisplay() {
		if ((ScreenIndex >= SCREEN_STATE_MAIN_B_TOP && ScreenIndex <= SCREEN_STATE_MAIN_B_END)
				|| (ScreenIndex >= SCREEN_STATE_MAIN_A_TOP && ScreenIndex <= SCREEN_STATE_MAIN_A_END) // ++,
				// --,
				// 150310
				// bwk
				|| (ScreenIndex >= SCREEN_STATE_MENU_TOP && ScreenIndex <= SCREEN_STATE_MENU_END) || (ScreenIndex == SCREEN_STATE_MAIN_CAMERA_GEAR)) {
	...
	public void CheckEHCUErr(int Data, int PopupOff) {
		 ...
		 * JoystickSteeringEnableFailCondition != 0x0000){ if(bEHCUErrPopup ==
		 * false){ if(JoystickSteeringEnableFailCondition != 0){
		 * if(JoystickSteeringEnableFailCondition !=
		 * OldJoystickSteeringEnableFailCondition){
		 * OldJoystickSteeringEnableFailCondition =
		 * JoystickSteeringEnableFailCondition; OldScreenIndex = ScreenIndex;
		 * showEHCUErr(); } }
		 * 
		 * } }
		 */
		...
		// {
		// OldJoystickSteeringEnableFailCondition =
		// JoystickSteeringEnableFailCondition;
		// bEHCUErrPopup = false;
		// }
		// else if(JoystickSteeringEnableFailCondition != 0xFFFF &&
		// JoystickSteeringEnableFailCondition != 0x0000){
		// if(bEHCUErrPopup == false){
	...
	// ++, 150211 bwk
	/*
	 * public void Checkrpm(int Data){ if(CAN1Comm.GetMultimediaFlag() == true
	 * && PressFnKey == 0) { // Log.d(TAG, "Player On"); PressFnKey = 1; } else
	 * if(CAN1Comm.GetMultimediaFlag() == false && PressFnKey == 1) { //
	 * Log.d(TAG, "Player OFF"); PressFnKey = 0; }
	 * 
	 * if(PressFnKey == 1) { if(Data == 0xFFFF) Data = 0; else if(Data > 9999)
	 * Data = 9999;
	 * 
	 * if(Data >= 1500) { if(HighrpmCount < 20) HighrpmCount++; LowrpmCount = 0;
	 * } else { if(LowrpmCount < 20) LowrpmCount++; HighrpmCount = 0; }
	 * 
	 * if(HighrpmCount >= 20 && HighrpmCount != 30) {
	 * CAN1Comm.ChangeMediatoHome(); HighrpmCount = 30; } else if(LowrpmCount >=
	 * 20 && LowrpmCount != 30) { CAN1Comm.ChangeHometoMedia(); LowrpmCount =
	 * 30; } } else { HighrpmCount = 0; LowrpmCount = 0; }
	 * 
	 * } // --, 150210 bwk
	 */
	// ///////////////////////////////////////////////////
	public void CheckAxleTempWarning(int _FrontAxleTempWarning, int _RearAxleTempWarning) {
		if (CAN1Comm.GetScreenTopFlag() == false) {
			if (((_FrontAxleTempWarning == CAN1CommManager.DATA_STATE_LAMP_ON) && (FrontAxleWarningFlag == false))
				...
				if (ScreenIndex != SCREEN_STATE_MAIN_B_TOP && ScreenIndex != SCREEN_STATE_MAIN_A_TOP) {
					...
					if (((ScreenIndex & SCREEN_STATE_FILTER) == SCREEN_STATE_MAIN_B_TOP) || ((ScreenIndex & SCREEN_STATE_FILTER) == SCREEN_STATE_MAIN_A_TOP)) {
		...
		} else if (ScreenIndex == SCREEN_STATE_MAIN_B_KEY_QUICKCOUPLER_POPUP_UNLOCKING3 || ScreenIndex == SCREEN_STATE_MAIN_A_KEY_QUICKCOUPLER_POPUP_UNLOCKING3
				|| (ScreenIndex == SCREEN_STATE_EHCUERR_POPUP && _EHCUErrorPopup.Safety_CPU_Error != 1) || ScreenIndex == SCREEN_STATE_MAIN_B_TOP
				|| ScreenIndex == SCREEN_STATE_MAIN_A_TOP) {
			{
				if (((_FrontAxleTempWarning == CAN1CommManager.DATA_STATE_LAMP_ON) && (FrontAxleWarningFlag == false))
						|| ((_RearAxleTempWarning == CAN1CommManager.DATA_STATE_LAMP_ON) && (RearAxleWarningFlag == false))) {
	...
	public void KeyButtonClick(final int Data) {
		...
		if (ScreenIndex == SCREEN_STATE_MAIN_CAMERA_KEY) {
			...
			// --, 150324 bwk
			// ++, 150310 bwk
		} else if (ScreenIndex == SCREEN_STATE_MAIN_A_KEY_QUICKCOUPLER_POPUP_LOCKING2 || ScreenIndex == SCREEN_STATE_MAIN_A_KEY_QUICKCOUPLER_POPUP_UNLOCKING2
				|| ScreenIndex == SCREEN_STATE_MAIN_A_KEY_QUICKCOUPLER_POPUP_UNLOCKING3) {
			...
			// }else if(ScreenIndex ==
			// SCREEN_STATE_MENU_MODE_HYD_WORKLOAD_WEIGHING_INIT1){
			// Log.d(TAG,"Click WeighingInit1 Key");
			// _WorkLoadWeighingInitPopup1.KeyButtonClick(Data);
			// }else if(ScreenIndex ==
			// SCREEN_STATE_MENU_MODE_HYD_WORKLOAD_WEIGHING_INIT2){
		...
		public void run() {
			...
			try {
				if (nSendCommandTimerIndex == 0) {
					...
					// CAN1Comm.setVolume(InternalSoundLevel); // ++, --, 150324
					// bwk
	...
	public class CommErrStopTimerClass extends TimerTask {
		...
		public void run() {
			...
			if (CommErrCount == 1000) {
				CAN1Comm.TxCMDToMCU(CAN1Comm.CMD_BUZ, CAN1Comm.BUZZER_ON); // Buzzer
				// On
			...
			runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					if (BackHomeCount == 0xff) {
						...
						if (((ScreenIndex & SCREEN_STATE_MAIN_FILTER) == 0x06000000)
								&& (((ScreenIndex & SCREEN_STATE_MAIN_KEY_FILTER) == 0x00400000)
										|| ((ScreenIndex & SCREEN_STATE_MAIN_KEY_FILTER) == 0x00500000)
	...
	public String GetVersionString(int VersionHigh, int VersionLow, int VersionSubHigh) throws NullPointerException {
		// public String GetVersionString(int VersionHigh, int VersionLow, int
		// VersionSubHigh, int VersionSubLow)throws NullPointerException{
		String strVersion;
		strVersion = Integer.toHexString(VersionHigh) + "." + Integer.toHexString(VersionLow) + "." + Integer.toHexString(VersionSubHigh);// +
																																			// "."
																							// +
		// Integer.toHexString(VersionSubLow);
		return strVersion;
	}
